...
Here you will find 6 exercises to learn different aspects of MdRQA and some questions to discuss.

1. Performing MdRQA on the Lorenz system. 
How do the results differ from those found using RQA? 
For what reasons?

```{r}
# set path...
setwd("C:/Users/alont/OneDrive/שולחן העבודה/Work/Leuphana/NLM2024/MdRQA Material/MdRQA_exercises")
...
# plot RP
RP <- res$RP
plotRP(RP,show_ticks = TRUE,unit = 500, labelx = "Time", labely = "Time", cols = "black", pcex = .5 , pch = 15)
```
...
plotRP(RP,show_ticks = TRUE,unit = 500, labelx = "Time", labely = "Time", cols = "black", pcex = .5 , pch = 15)
```
Exercise 4: The effect of data normalization in MdRQA.

Load the file R.csv and add it to the Lorenz-data.frame as fourth variable.
However,now use the scale()-function to z-score the data from the R.csv-file before adding it as fourth variable to the Lorenz data frame.
...
plotRP(RP,show_ticks = TRUE,unit = 500, labelx = "Time", labely = "Time", cols = "black", pcex = .5 , pch = 15)
```
6. Use Lagged MdRQA to find the lags in the messed Lorenz system.
Re-do the analysis from Exercise 5, but use the lagged MdRQA wrapper.
Here we resample the Lorenz system for a quicker computation.

First run the Lagged MdRQA function:
```{r}
...
Choose some value for the „maxlag“ parameter between 10-30.
Investigate the lag structure compared to the Lorenz system plot.

```{r}
# set path...
setwd("C:/Users/alont/OneDrive/שולחן העבודה/Work/Leuphana/NLM2024/MdRQA Material/MdRQA_24/MdRQA_exercises")
